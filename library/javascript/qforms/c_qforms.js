var _jsver = 11;
for (var z = 2; z < 6; z++) document.write("<script language=\"JavaScript1." + z + "\">_jsver = 1" + z + ";</scr" + "ipt>");

function _a() {
    this.version = "137g";
    this.instances = 0;
    this.objects = new Object();
    this.librarypath = document.currentScript.getAttribute('libpath');
...
function _a_setLibraryPath(path) {
    //if (path.substring(path.length - 1) != 'index.html') path += 'index.html';
...
function _f_disabled(s) {
    ...
    if (!objExists) this.locked = status;
        ...
        if (!!this.obj[0] && this.type.indexOf("select") == -1)
            for (var i = 0; i < this.obj.length; i++) this.obj[i].disabled = status;
        else this.obj.disabled = status;
    }
    return true;
}
...
function _f_getValue() {
    ...
    if (type == "select") {
        ...
        if (!!this.obj[0]) {
            for (var i = 0; i < this.obj.length; i++)
                if (this.obj[i].checked) value[value.length] = this.obj[i].value;
    ...
    if (type == "select") {
        ...
        if (!this.container) {
            ...
            if (bSelectOne && bLookForFirst) {
                if (this.defaultValue == orig)
                    if (this.obj.length > 0) this.obj[0].selected = true;
...
function _addValidator(name, fn, alwaysRun) {
    ...
    for (var a = 0; a < qFormAPI.validators.length; a++)
        if (qFormAPI.validators[a] == name) return alert("The " + name + " validator has already been loaded.");
...
function _f_checkForErrors() {
    ...
    for (var i = 0; i < this._queue.validation.length; i++)
        if (!eval(this._queue.validation[i])) break;
